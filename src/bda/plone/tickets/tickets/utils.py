...
def find_tickets(context):
    ...
    if language:
        brains = catalog(Subject='ticket', Language=language, sort_on='getObjPositionInParent')
    else:
        brains = catalog(Subject='ticket', sort_on='getObjPositionInParent')
    return brains
...
def extract_tickets(ret, request):
    if request != None:
